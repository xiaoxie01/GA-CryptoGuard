...
Flow: `trellis-implement` -> `trellis-check` -> for high-risk CryptoGuard changes run `crypto-guard-final-seal` -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
...
- High-risk CryptoGuard implementation claimed complete -> `crypto-guard-final-seal`.
...
### CryptoGuard Project Overlay

Read `.trellis/spec/guides/crypto-guard-delivery.md` for any CryptoGuard change involving market data, decisions, risk, orders, positions, evaluations, evolution, persistence, migrations, scheduling, notifications, or reports.

- These changes are high risk and require `prd.md`, `design.md`, and `implement.md`.
- After `trellis-check`, run the `crypto-guard-final-seal` skill before spec update or commit.
- Every finding is mandatory, including P2, recommended fixes, weak tests, diagnostic gaps, and documentation drift.
- Keep implementation complete, final seal complete, production ready, and production recovered as separate states.
- Production mutation and service restart require `/trellis:crypto-guard-release` plus explicit user confirmation.
